...
function fileorganizer_pro_license(){
	...
	$resp = wp_remote_get(FILEORGANIZER_API.'license.php?license='.$license.'&url='.rawurlencode(site_url()), array('timeout' => 30));
	
	if(!is_array($resp)){
		fileorganizer_notify(__('The response was malformed<br>'.var_export($resp, true)), 'error');
		return;
	}
	...
	update_option('fileorganizer_license', $json);
