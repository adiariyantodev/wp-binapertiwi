...
function pfx_load_license($parent = 0){
	...
	// Load license
	$popularfx['license'] = get_option($license_field, []);
	...
	if( !empty($popularfx['license']) && !empty($popularfx['license']['license']) && (time() - $last_update) >= 86400 ){
		
		$resp = wp_remote_get($license_api_url.'license.php?license='.$popularfx['license']['license'].'&url='.rawurlencode(site_url()));
	...
	if(empty($popularfx['license']) || empty($popularfx['license']['active'])){
		...
		if(function_exists('softaculous_pro_load_license')){
			...
			$popularfx['license'] = get_option('softaculous_pro_license', []);
